...
{
    /**
     * @brief comapre shared_ptr<Citizen> to int by Citizen::getId()
     *
     * @param citizen
     * @param id
     * @return true
     * @return false
     */
    bool operator==(const shared_ptr<Citizen> &citizen, const int &id);
    ...
    {
        //Check existance
        Manager* manager = getManagerById(manager_id).get();
        Workplace* workplace = getWorkplaceById(workplace_id).get();

        workplace->fireManager(manager_id);
        manager->fireAllEmployees(0);
    }

    int City::getAllAboveSalary(ostream &os, const int salary_bar) const
    {
        int count = 0;
        vector<Citizen *> vector_union;
        for (const shared_ptr<Employee> &employee : employees_t)
        {
            vector_union.push_back(employee.get());
        }
        for (const shared_ptr<Manager> &manager : managers_t)
        {
            vector_union.push_back(manager.get());
        }
        sort(vector_union.begin(), vector_union.end(), Comparator());
        for (Citizen *citizen : vector_union)
        {
